...
During gather, the current poster remains stable and readable. At slice start, the prior slot becomes visual-only `is-outgoing` and its image is immediately `aria-hidden`; the incoming slot becomes the only semantic `is-active` poster. There may be at most one outgoing continuity layer, and there must never be a sampled full/compact frame with neither an active nor outgoing visible image. At slice settlement, the outgoing layer and slit are already at opacity `0` before their transient classes are removed. Cancellation, retry, freeze, switching an in-flight scene to `reduce`, and destroy clear every transient class and duration property so a stale completion cannot change a later run. A `full <-> compact` change immediately updates the controller profile, root dataset/CSS profile, and particle profile; `particleField.setProfile(...)` immediately settles the active gather or scatter command. It does not increment the poster generation, abort or reorder the current scheduler sleeps, clear the current handoff classes, or requeue the current item. The current DOM handoff completes on its already scheduled sleeps, and the next scene starts with the new profile and newly selected timing.
...
The full LTR outgoing cover uses `translate3d(2.4%, -0.6%, 0) rotateX(1.2deg) scale(0.992)`; full RTL uses `translate3d(-2.4%, -0.6%, 0) rotateX(-1.2deg) scale(0.992)`. Compact LTR uses `translate(14px, -2px) scale(0.992)` and compact RTL uses `translate(-14px, -2px) scale(0.992)`, with no perspective. The incoming cover remains complete and uncropped with `object-fit: contain`; the slit reveal may change only opacity, transform, and `clip-path`.
